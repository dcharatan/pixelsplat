...
def generate_image_grid(
    ...
    image_width = (FIGURE_WIDTH - (n - 3) * MARGIN) / (n - 2.5)
    figure_height = num_rows * image_width + (num_rows - 1) * MARGIN + TEXT_SIZE
    mini_image_width = (image_width - 2 * MARGIN) / 3
    ...
    # Add the third context image.
    for row, (_, _, image, *_) in enumerate(images):
        if image is None:
            image = torch.ones((3, 128, 128), dtype=torch.float32) * 0.5
        ...
        image = svg.Image(
            ...
            href=encode_image(image, "jpeg"),
            x=0,
            y=offset * row + TEXT_SIZE + 2 * (MARGIN + mini_image_width),
        )
        fig.elements.append(image)

    for row, row_images in enumerate(images):
        for col, image in enumerate(row_images[3:]):
            if image is None:
                image = torch.ones((3, 128, 128), dtype=torch.float32) * 0.5
...
def generate_image_comparison():
    ...
    cfg_dict = compose(
        ...
        overrides=["+experiment=re10k_3_view", "+evaluation=re10k_3_view"],
    )
    set_cfg(cfg_dict)
    ...
    for highlighted in tqdm(cfg.evaluation.highlighted, "Loading data"):
        ...
        context_a, context_extra, context_b = example["context"]["image"]
        gt = example["target"]["image"][0]
        row = [context_a, context_extra, context_b, gt]
